  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Gate Pass Request</h1>
          <p className="text-sm text-gray-500">RAMS/GPR/01 — Manager raises → Stores arranges vehicle → Accounts processes payment.</p>
        </div>
        {canCreate && (
      ...
      {loading ? (
        ...
        <Card className="text-center text-gray-500 py-10">
          {activeTab === 'ALL'
            ...
            : `No gate pass requests in ${STATUS_META[activeTab]?.label.toLowerCase()}.`}
        </Card>
      ) : (
...
function DetailModal({ gatePass: initial, onClose, onAction }) {
  ...
  const canAccountsApprove = g.status === 'PENDING_ACCOUNTS' && (role === 'ACCOUNTING' || isAdmin);
            ...
            <table className="w-full text-xs">
              <thead className="bg-gray-50">
                <tr className="text-left text-gray-600">
                  ...
                  <th className="px-2 py-1.5">Pass type</th>
