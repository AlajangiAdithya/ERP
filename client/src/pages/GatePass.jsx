...
export default function GatePass() {
  ...
  const canCreateInward = user?.role === 'MANAGER' || user?.role === 'STORE_MANAGER' || user?.role === 'ADMIN';
  ...
  return (
    <Modal isOpen onClose={onClose} title={isInward ? 'Record Inward Gate Pass (FIM)' : 'Gate Pass Request'} size="full">
      <div className="space-y-6">
        ...
        {isInward ? (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 p-3 bg-blue-50 border border-blue-200 rounded">
            ...
            <Input label="Customer contact" value={customerContact} onChange={e => setCustomerContact(e.target.value)} placeholder="Phone / email" />
          </div>
        ) : (
              ...
              <tbody>
                {items.map((it, idx) => (
                  <tr key={idx} className="border-t border-gray-100 align-top">
                    ...
                    <td className="px-2 py-2">
                      {!isInward && it.itemPassType === 'DELIVERY_CHALLAN' ? (
                        <select className={cellInput}
      ...
      <div className="space-y-4">
        ...
        {isInward ? (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm p-3 bg-blue-50 border border-blue-200 rounded">
            ...
            {g.customerContact && <Field label="Customer contact" value={g.customerContact} />}
        ...
        <div>
          ...
          {isInward && g.status === 'PENDING_ACCEPTANCE' && (
