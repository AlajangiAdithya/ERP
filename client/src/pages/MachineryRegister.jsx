...
import {
  Wrench, Flame, Plus, Pencil, Trash2, FileText, Upload, AlertTriangle,
...
// ──────────────────────────────────────────────────────────────
// Machinery + Fire Extinguisher register page.
// SAFETY (department) and Unit-5 users edit; everyone else views.
// Tabs follow the same pattern as Products page (RAPS / FIM).
// ──────────────────────────────────────────────────────────────
...
export default function MachineryRegister() {
  ...
  const reload = async () => {
    ...
    try {
      ...
      setMachinery(m.data.items || []);
      setFires(f.data.items || []);
      setCanWrite(Boolean(m.data.canWrite));
    } catch (err) {
  ...
  const filteredFires = useMemo(() => {
    ...
    return fires.filter((f) => {
      if (placeFilter && f.unit !== placeFilter) return false;
      if (!q) return true;
      return [f.type, f.rapsId, f.capacity, f.unit, f.location]
        .filter(Boolean)
        .some((v) => v.toLowerCase().includes(q));
    });
  }, [fires, search, placeFilter]);
  ...
  return (
    <div className="space-y-5">
      <PageHero
        title="Machinery & Safety Register"
        eyebrow="Safety / HSE"
        subtitle="Master list of shop-floor machinery (with AMC status) and the active fire-extinguisher inventory. Editable by Safety and Unit-5; others view-only."
        icon={Wrench}
        actions={canWrite && (
          tab === 'machinery'
      ...
      {/* Tab bar — mirrors Products page tab style */}
      <div className="flex gap-1 border-b border-gray-200">
        <button
          onClick={() => { setTab('machinery'); setPlaceFilter(''); }}
        ...
        >
          <Wrench size={15} /> Machinery
          <span className="ml-1 px-1.5 py-0.5 text-[10px] rounded-full bg-gray-100 text-gray-600">{machinery.length}</span>
        </button>
        <button
          onClick={() => { setTab('fire'); setPlaceFilter(''); }}
          className={`px-5 py-2.5 text-sm font-medium border-b-2 -mb-px transition-colors flex items-center gap-2 ${
            tab === 'fire'
              ? 'border-navy-700 text-navy-700'
              : 'border-transparent text-gray-500 hover:text-gray-700'
        ...
        >
          <Flame size={15} /> Fire Extinguishers
          <span className="ml-1 px-1.5 py-0.5 text-[10px] rounded-full bg-gray-100 text-gray-600">{fires.length}</span>
        </button>
      </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-4">
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder={tab === 'machinery' ? 'Search machinery, RAPS ID, make…' : 'Search type, RAPS ID, location…'}
          ...
          <Select value={placeFilter} onChange={(e) => setPlaceFilter(e.target.value)} className="w-full sm:w-56">
            <option value="">{tab === 'machinery' ? 'All Units' : 'All Locations'}</option>
            {(tab === 'machinery' ? places : units).map((p) => <option key={p} value={p}>{p}</option>)}
          </Select>
        </div>
