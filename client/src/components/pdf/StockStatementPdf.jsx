      ...
      <Page size="A4" style={styles.page} orientation="landscape">
        ...
        <View style={styles.table}>
          <View style={styles.row}>
            <View style={[styles.cellHeader, { width: '14%' }]}><Text>Date</Text></View>
            <View style={[styles.cellHeader, { width: '28%' }]}><Text>Product</Text></View>
            <View style={[styles.cellHeader, { width: '12%' }]}><Text>SKU</Text></View>
            <View style={[styles.cellHeader, { width: '9%' }]}><Text>Type</Text></View>
            <View style={[styles.cellHeader, { width: '9%' }]}><Text>Qty</Text></View>
            <View style={[styles.cellHeader, { width: '6%' }]}><Text>UOM</Text></View>
            <View style={[styles.cellHeader, { width: '11%' }]}><Text>Ref Type</Text></View>
          ...
          {movements.length === 0 ? (
            ...
            <View key={m.id || idx} style={styles.row} wrap={false}>
              <View style={[styles.cell, { width: '14%' }]}><Text>{formatDateTime(m.createdAt)}</Text></View>
              <View style={[styles.cell, { width: '28%' }]}><Text>{m.product?.name || '—'}</Text></View>
              <View style={[styles.cell, { width: '12%' }]}><Text>{m.product?.sku || '—'}</Text></View>
              <View style={[styles.cell, { width: '9%' }]}><Text>{m.type}</Text></View>
              <View style={[styles.cell, { width: '9%' }]}><Text>{m.quantity}</Text></View>
              <View style={[styles.cell, { width: '6%' }]}><Text>{m.product?.unit || '—'}</Text></View>
              <View style={[styles.cell, { width: '11%' }]}><Text>{m.referenceType || '—'}</Text></View>
