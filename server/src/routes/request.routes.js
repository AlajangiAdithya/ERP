...
const { generateSequentialNumber, generateGatePassNumber, paginate, applyDateFilter, isUniqueViolation, deptForRole, validateWorkOrderLink } = require('../utils/helpers');
...
router.post('/offsite/gatepass', authenticate, authorize('STORE_MANAGER', 'ADMIN'), async (req, res) => {
  try {
    ...
    const unit = await prisma.unit.findUnique({ where: { id: unitId }, select: { id: true, name: true, code: true, isOffsite: true } });
    ...
    // Per-unit gate-pass number RAPS/GP/<UNIT>/<FY>/<N> (destination = the offsite unit).
    const passNumber = (rawPassNumber || '').trim() || await generateGatePassNumber(prisma, unit.code || unit.name);
